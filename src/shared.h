...
/* ── KKW parameters ────────────────────────────────────────────────────────
 * N_PARTIES = 16, NUM_ROUNDS = 32 → soundness (1/16)^32 = 2^{-128}.
...
extern const int NUM_ROUNDS;   /* 32 */
...
static inline uint32_t tape_u(const unsigned char *tape, int g)
{ return tape_get32(tape, g * 4); }
static inline uint32_t tape_v(const unsigned char *tape, int g)
{ return tape_get32(tape, ySize * 4 + g * 4); }
static inline uint32_t tape_w(const unsigned char *tape, int g)
{ return tape_get32(tape, 2 * ySize * 4 + g * 4); }
