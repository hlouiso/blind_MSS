...
int main(int argc, char *argv[])
{
    ...
    for (int round = 0; round < NUM_ROUNDS; round++) {
        ...
        for (int byte = 0; byte < INPUT_LEN; byte++) {
            ...
            for (int p = 0; p < N_PARTIES - 1; p++) xor_val ^= x_shares[round][p][byte];
...
#pragma omp parallel for
    for (int round = 0; round < NUM_ROUNDS; round++) {
        ...
        for (int p = 0; p < N_PARTIES; p++) {
            ...
            if (!tapes[p]) { tape_ok = false; break; }
        ...
        building_views(as[round], m_hat, pk_seed,
                       x_shares[round], tapes,
                       zs[round]->broadcast, zs[round]->aux);

        /* Verify circuit output XOR */
        for (int j = 0; j < 8; j++) {
            uint32_t xorv = 0;
            for (int p = 0; p < N_PARTIES; p++) xorv ^= as[round]->yp[p][j];
            if (xorv != pubout[j]) error = true;
        }
...
#pragma omp atomic
    ...
    for (int round = 0; round < NUM_ROUNDS; round++) {
        ...
        for (int j = 0; j < N_PARTIES - 1; j++) {
            int orig = (j < e) ? j : j + 1;
            memcpy(zs[round]->x_revealed + (size_t)j * INPUT_LEN,
                   x_shares[round][orig], INPUT_LEN);
        }
