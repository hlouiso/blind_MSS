...
/*
 * Halevi–Micali statistically-hiding commitment over GF(2^128), laid out exactly
 * as in https://github.com/diegode/blind-longfellow so the security analysis carries over.
 *
 *   y   = SHA256(r_1 || ... || r_6)          (hashes the n=6 nonces ONLY)
 *   b_k = m̂_k + Σ_i a_{k,i} · r_i  over GF(2^128)   (k = 0,1; one line per
 *                                                     16-byte half of m̂)
 *   com = a || b || y                        (the commitment, 256 bytes)
 *   d   = SHA256(com)                        (the digest the signer certifies)
 *
...
#define HM_Y_BYTES 32                            /* SHA-256 digest */
